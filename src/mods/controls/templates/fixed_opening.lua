...
function fixedOpening.prepare(_, room)
    if room.kind ~= "Opening" then
        fail(room, "must have kind 'Opening'")
    end
    if room.incomingReward.kind ~= "countedChoice" then
        fail(room, "requires a countedChoice incoming reward")
    end
    if room.encounterProfileKey ~= "F_Opening" and room.encounterProfileKey ~= "N_Opening" then
        fail(room, "requires an opening encounter profile")
    end
