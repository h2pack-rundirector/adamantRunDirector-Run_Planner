...
local function cachedOpts(control, cacheName, rowIndex, base, source)
...
local function drawTargetRow(draw, control, rowIndex)
    ...
    drawDropdownLine(draw, "Biome", control:biomeField(rowIndex), decoratedOpts(
        ...
        rowIndex,
        "BiomeKey",
        cachedOpts(control, "_biomeOptsByRow", rowIndex, BIOME_OPTS, control:biomeOptions(rowIndex))
    ), function()
    ...
    if control:shouldRenderRoom(rowIndex) then
        drawDropdownLine(draw, "Room", control:roomField(rowIndex), decoratedOpts(
            ...
            rowIndex,
            "RowIndex",
            cachedOpts(control, "_roomOptsByRow", rowIndex, ROOM_OPTS, control:roomOptions(rowIndex))
        ), function()
    ...
    if control:shouldRenderVariant(rowIndex) then
        drawDropdownLine(draw, "Type", control:variantField(rowIndex), decoratedOpts(
            ...
            rowIndex,
            "VariantKey",
            cachedOpts(control, "_variantOptsByRow", rowIndex, VARIANT_OPTS, control:variantOptions(rowIndex))
        ), function()
...
function ui.create(fields, instance)
    ...
    function control.biomeField(_, rowIndex)
        return fields.Targets:get(rowIndex, "BiomeKey")
    end

    function control.roomField(_, rowIndex)
        return fields.Targets:get(rowIndex, "RowIndex")
    end

    function control.variantField(_, rowIndex)
