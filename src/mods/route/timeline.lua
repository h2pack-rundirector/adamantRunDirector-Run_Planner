...
local function nextKnownCounter(previous, valueKey, costKey, startValue)
    if previous == nil then
        return startValue, true
    end
    if previous[valueKey .. "Known"] == false or previous[valueKey] == nil or previous[costKey] == nil then
        return nil, false
    end
    return previous[valueKey] + previous[costKey], true
end

function timeline.nextBiomeRowCounters(instance, previous, target)
    local biomeDepthCache, biomeDepthCacheKnown = nextKnownCounter(
        previous,
        "biomeDepthCache",
        "biomeDepthCacheCost",
        timeline.biomeDepthCacheStart(instance)
    )
    local biomeEncounterDepth, biomeEncounterDepthKnown = nextKnownCounter(
        previous,
        "biomeEncounterDepth",
...
local function routeWalkContext(route, routeState, biomeState, routeBiomeIndex, biomeKey, row, rowCost)
    return {
        ...
        biomeEncounterDepthKnown = row and row.biomeEncounterDepthKnown or nil,
