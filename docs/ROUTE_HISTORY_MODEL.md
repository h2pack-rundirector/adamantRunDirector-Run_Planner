...
Current reward information is inline metadata on room entries and generated
topology exits. Standalone `rewardOffer` and `rewardPick` events can be added
later as an index over the same spine when reward bag simulation or query
performance needs them.
...
Room entries are the route spine. Generated next-choice facts hang off the room
that generated them:
...
The repetition is intentional. The current room entry records what the game
generated next; the next room entry records what the player actually entered.
Rewards, NPCs, and features are enrichments on either the entered room or a
generated branch, not competing route-spine entries.

Generated reward branches use the same rule when the reward declaration exposes
multiple branch-gated offers. For F/G/H/P preboss, the room before preboss
records both generated branches, such as `PrebossShop` and
`PrebossFreeReward`, with one branch marked as picked. The preboss room entry
then records the actual branch outcome, such as bought shop items or the free
reward that was taken. A shop-only preboss declaration, such as Q, stays a
single generated exit because there is no second branch offer to model.
...
3. Extend the selected snapshot and adapter to cover fixed-linear sibling
   topology facts.
4. Add reward bag/query indexes over the fixed-linear room spine when the bag
   simulator needs faster reward lookup.
