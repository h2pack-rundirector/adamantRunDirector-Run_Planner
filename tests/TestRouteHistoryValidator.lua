...
function TestRunPlannerRouteHistoryValidator.testValidatorAllowsFieldsBridgeCrowdedByTwoForcedDoors()
    local route = {
        key = "Underworld",
        biomes = { "H" },
    }
...
function TestRunPlannerRouteHistoryValidator.testValidatorRejectsFieldsBridgeCrowdedByOneForcedDoor()
    ...
    lu.assertEquals(result.invalids[1].code, "forced_topology_pressure_unresolved")
    lu.assertEquals(result.invalids[1].topologyForceLabel, "Echo")
    lu.assertEquals(result.invalids[1].generatedCount, 1)
    lu.assertEquals(result.invalids[1].requiredGeneratedCount, 2)
end

function TestRunPlannerRouteHistoryValidator.testValidatorAllowsForceGroupDeferredBySaturatedForcedDoors()
    local catalog = h.loadCatalog()
    local history = routeHistory.create()
    emitRoom(history, 6, {
        biomeKey = "F",
