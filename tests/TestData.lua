...
local function roomStoreReward(rewardStore, opts)
    ...
    if opts.allowedRewardTypes ~= nil then
        reward.allowedRewardTypes = opts.allowedRewardTypes
    end
    return reward
end
...
local function fieldsCagesReward(rewardStore)
    return {
        kind = "fieldsCages",
...
function TestRunPlannerData.testBiomeDefinitionsExposeVanillaDepthScope()
    ...
    lu.assertEquals(biomes.lookup.G.slotLayout.routeStartDepth, 2)
    ...
    lu.assertEquals(biomes.lookup.O.slotLayout.routeStartDepth, 2)
    ...
    lu.assertEquals(biomes.lookup.P.slotLayout.routeStartDepth, 2)
    ...
    lu.assertEquals(biomes.lookup.Q.slotLayout.routeStartDepth, 2)
...
function TestRunPlannerData.testBiomeDefinitionsDeclareDepthSpecials()
    ...
    local gIntro = biomes.lookup.G.slotLayout.special[1]
    ...
    local pIntro = biomes.lookup.P.slotLayout.special[1]
    ...
    local qIntro = biomes.lookup.Q.slotLayout.special[1]
    ...
    local oIntro = biomes.lookup.O.slotLayout.special[1]
...
function TestRunPlannerData.testShopProfilesModelAllStoreOptions()
    local data = dofile("src/mods/data.lua")
    local biomes = data.loadBiomes(testImport)

    local standard = biomes.shopProfiles.WorldShop
    lu.assertEquals(standard.optionSlots[1].key, "Boon")
    lu.assertEquals(standard.optionSlots[2].key, "MajorNonBoon")
    lu.assertEquals(standard.optionSlots[3].key, "Minor")
    lu.assertEquals(#standard.optionSlots, 3)

    local tartarus = biomes.shopProfiles.I_WorldShop
    lu.assertEquals(#tartarus.optionSlots, 5)
    lu.assertEquals(tartarus.optionSlots[1].key, "Group1Offer1")
    lu.assertEquals(tartarus.optionSlots[1].label, "Priority Power")
    lu.assertEquals(tartarus.optionSlots[5].key, "Group5Offer1")
    lu.assertEquals(tartarus.optionSlots[5].label, "Resource")

    local summit = biomes.shopProfiles.Q_WorldShop
    lu.assertEquals(#summit.optionSlots, 6)
    lu.assertEquals(summit.optionSlots[1].key, "Group1Offer1")
    lu.assertEquals(summit.optionSlots[2].key, "Group1Offer2")
    lu.assertEquals(summit.optionSlots[2].label, "Primary Power B")
    lu.assertEquals(summit.optionSlots[2].storeGroup, 1)
    lu.assertEquals(summit.optionSlots[2].offerIndex, 2)
    lu.assertEquals(summit.optionSlots[6].key, "Group5Offer1")
end
